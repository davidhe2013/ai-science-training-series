...
import time
t0 = time.time()
parser = argparse.ArgumentParser(description='Horovod tensorflow2_minist')
...
# HVD-3 Loading data according to rank ID and ajusting the number of time steps
...
#optimizer = tf.keras.optimizers.Adam(learning_rate=0.00005*hvd.size())  # from train_resnet34._hvd.py
...
for ep in range(args.epochs):
  ...
  # HVD-7: Checkpointing on root rank.  Save the network after every epoch:
    if (hvd.rank()==0):
        checkpoint.save("Homework/model")
